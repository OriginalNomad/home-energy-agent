...
- [x] **LP optimiser — wire in `solar_unreliable` flag**: zeros solar series in LP when flag set. 14/14 optimizer tests pass. `optimizer_vs_deterministic` now meaningful on cloudy days. (2026-06-07)
...
- [ ] **Re-architecture Phase 6 — slim the prompt**: remove arithmetic the LLM no longer needs (det layer authoritative + Phase 7 skips most cycles). Unify `hours_to_cheap_end` prose onto the scale-free model. Lower priority now Phase 7 is done — cost savings already captured.
- [ ] **Verify Phase 7 on Pi** — after next overnight cycle, check HA notifications show one-liners for `overnight_hold_wait_for_sponge` and full LLM narrative for first cycle of a new rule (transition). Confirm in decisions.jsonl that skipped cycles have `summary` set to templated string.
- [ ] **Mac Studio git sync** — set up HA `shell_command: git_pull_home_energy` + add step to `/finish` skill to call it. Needs: HA_TOKEN in cloud session env + HA reload. Deferred until back at desktop.
- [x] **Re-architecture Phase 7 — selective narrative**: `ROUTINE_RULES` gate skips LLM on 7 routine rules when unchanged from previous cycle. Full log/JSONL/notification still written with templated one-liner. ~70% call reduction (~$97→~$30/month). Implemented before Phase 6 — gate is orthogonal to prompt content. (2026-06-07)

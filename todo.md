...
- [ ] **Re-examine `peak_solar_cover_survival` in the `kwh_needed_85 <= 0` branch (design, raised 2026-07-07)** — when net solar is *forecast to cover* the gap to 85%, charging from grid at the current price (observed: ~20¢, 5% SoC, 8am) buys ~10.8 kWh (5%→85%) that the sun was about to deliver for ~free, and then that solar exports at a poor/negative FIT or clips. Only the home-load portion during the drain is a genuine wash; the top-up is a real ~$1–2/morning arbitrage loss. Hitting the 5% floor is itself harmless (grid covers house load). The rule is effectively **insurance against solar-forecast failure** — but the deadline escalators (`peak_deadline_autonomous`, `peak_sponge_go_hard`) already catch that failure later in the day. Questions: (1) should the survival branch **hold** (mirror `peak_early_morning_hold`) instead of charge when `kwh_needed_85 <= 0`, trusting the downstream escalators? (2) the `forward_min ≤ price − 5.0` gate is a risk premium, not an arbitrage break-even — it should scale with the top-up size (10 kWh vs 2 kWh) and forecast reliability, not a flat 5¢. The code comment at `energy_agent.py:1551-1553` ("charging now is strictly cheaper") is wrong and should be corrected when this is revisited. **The LP already has the tunable form of this insurance**: with `risk=0` (`optimizer.py:62`) it credits forecast solar in the power balance and holds (buys nothing at 20¢, lets solar fill the battery free — `mpc_hold`/`mpc_solar_only`), while the `risk` knob (`solar × (1−0.5·risk)`, `load × (1+0.3·risk)`, lines 185-188) dials continuously toward pre-charging as insurance. `peak_solar_cover_survival` is effectively pinned at a fixed high-risk point; the LP lets you set it. Open question for cutover: does a modest `risk` (~0.1–0.2) buy back enough robustness to trust the LP's cheaper answer? See "LP to control path" below.

...
- **Verification (added 2026-06-02; re-banded 2026-07-03):** `agent/log_daily_energy.py` records each peak day's outcome to `agent/daily_energy.jsonl`. The metric matches the bill exactly — the **peak clock-aligned 30-minute average import (kW)** over 3–9pm, *not* kWh or instantaneous peak. The monthly bill is driven by the single worst day's peak, so `sensor.demand_window_monitor` (pushed via REST API, no HA config change) surfaces this month's running max + a rolling per-day history for the dashboard cards.
  - **Three-way classification (`classify_demand()`, from 2026-07-03), banded by dollar impact at the EA116 rate (~$11.5479/kW/month, from the Jun 2026 bill):** `pass < 0.5 kW` (< ~$6/mo) · `marginal 0.5–1.5 kW` (~$6–17/mo) · `breach ≥ 1.5 kW` (≥ ~$17/mo). Each day's record carries `status` + `cost_est_dollars`.
  - **Why re-banded:** the old single 0.10 kW pass threshold flagged unavoidable Powerwall regulation-lag days (any dinner-time load step draws briefly from grid before the battery ramps — nets ~0.1–0.25 kW, i.e. ~$1–3/mo) with the *same* red "breach" as a genuine sustained control failure. A literal-zero demand charge is unreachable (the charge is the max over ~360 window-intervals/month, and each dinner half-hour nets slightly positive); the real, preventable cost is a **sustained** high-import half-hour like the Jun 2 stranded-reserve breach (2.69 kW = $31.11). Banding by $ keeps the alarm on that class only. *(Diagnosed 2026-07-03 from the Jul 3 "breach" — 0.208 kW @ 18:00 ≈ $2.40/mo — which was not a control failure; battery discharged correctly at reserve 5%.)*

...
**Morning-ramp guard (2026-07-15) — the "9am cliff" fix:** the accuracy-based `solar_unreliable` flag was a *step function* at 9am. Between 9:00 and ~10:00 the flat roof is still ramping and the inverter reads well below Solcast's hourly figure (e.g. 0.1 kW actual vs 1.5 kW forecast → `unreliable`). A single such sample used to **zero a genuine full-day forecast** and drive a needless autonomous grid-charge to 85% (observed live 2026-07-12: 20% SoC, 9:30am, 17 kWh still forecast → battery grid-charged to 99%, then stranded in autonomous and starved the EV of surplus). Fix: during the ramp window (`SOLAR_START_HOUR ≤ now < SOLAR_RAMP_SETTLE_HOUR = 10.0`), a `poor`/`unreliable` reading is treated as ramp lag and **does not** flip `solar_unreliable` while Solcast still forecasts a healthy day (`remaining_today > SOLAR_HEALTHY_REMAINING_KWH = 2.0`). This mirrors the pre-10am guard `_detect_zero_solar` already has, so the two solar-distrust paths agree. After 10am, or when `remaining_today ≤ 2 kWh`, the `unreliable` label is trusted exactly as before. Covered by `test_solar_unreliable_ramp_guard_*` in `test_decision.py`. *Tuning note:* if winter data shows the flat roof is still ramping past 10am, nudge `SOLAR_RAMP_SETTLE_HOUR` to ~10.5 (and update `test_solar_unreliable_after_9am`, which pins the 10:00 boundary).

...
Moved to `NETWORK.md` (gitignored) — contains IPs, MACs, and device-specific details.
The key architectural facts are:

- **Double-NAT**: two routers in series (ISP modem → mesh router). The inverter is on the ISP side (cloud API, no LAN access needed); everything else is on the mesh side.
- **Pi and Powerwall must be on the same subnet** — the Powerwall integration is local.
- **Reserve DHCP addresses** for every device HA depends on — 1-hour leases cause silent breakage after power cuts.

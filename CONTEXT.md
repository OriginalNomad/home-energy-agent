...
- **Rule 27 — manual override** (`input_boolean.agent_manual_override` → **renamed 2026-08-05 to `input_boolean.agent_active`, ON = active**): see energy_rules.md. Suppresses the rule layer's commands (including HOLD, which would otherwise clear reserve to 5% and undo a manual setting) while paused. Auto-resumes after 12h, fails safe toward active. Does *not* suspend the Rule 2 demand-window guard or any HA automation.
